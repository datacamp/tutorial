...
You can convert this code chunk into a fiddle to experiment. Simply specify an `ex` identifier:
...
The resulting HTML file will contain an iframe that looks like this, where people can experiment. On the left, there is a script editor. When you hit Run, your code is executed in the console. You can also directly experiment inside this console.

```{r eval = FALSE, include = FALSE, ex="play_around"}
a <- 2
b <- 3
```

To not clutter the code example, you might want to pre-load some data or load some packages beforehand. You can do this by specifying a chunk with `type = 'pre-exercise-code'`. Make sure you specify the same `ex` label to tie the two code blocks together:
